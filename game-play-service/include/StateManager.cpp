...
void StateManager::beginUserInitiatedSignIn() {
  LOGI("Check StartAuthorizationUI");
...
void StateManager::reportAchievement(const char *achievement_id,
		uint16_t currentStep,
		uint16_t maxstep,
		bool isPercent) {
	if(sGameServices->IsAuthorized()) {
		float percent = currentStep;
		if(isPercent) {
			percent = ((1.0f * currentStep)/maxstep) * 100.0f;
		}
		sGameServices->Achievements().Increment(achievement_id, percent);
	}
	else {
